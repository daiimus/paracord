...
Options:
  ...
  --skip-meowed         Skip meowed messages (current and legacy format)
```
...
| `skip_meowed` | false | Skip meowed messages (both current and legacy format) |
...
> **Legacy compatibility:** Messages meowed with v3.2.0 or earlier used the format `Meow Meow Meow Meow`. The `--skip-meowed` filter recognizes both the old and new formats.
...
The `--meow` CLI flag overrides whatever is in the config file. Messages that are already meowed (current or legacy format) are skipped on subsequent passes.
...
The `--skip-meowed` CLI flag overrides the config setting. Messages are matched by exact content against both the current format (`**Meow, Meow.**` x4 lines) and the legacy format (`Meow Meow Meow Meow`).
