...
Options:
  ...
  --skip-meowed         Skip messages already containing "Meow Meow Meow Meow"
```
...
| `skip_meowed` | false | Skip messages containing "Meow Meow Meow Meow" (preserve meowed messages) |
...
Meow mode overwrites every message's content with "Meow Meow Meow Meow" before (optionally) deleting it. This is useful as a belt-and-suspenders approach: even if a deletion fails or you choose to leave messages standing, the original text is gone.
...
| `edit_and_delete` | Edit message to "Meow Meow Meow Meow", then delete it |
| `edit_only` | Edit message to "Meow Meow Meow Meow" and leave it standing |
...
The `--meow` CLI flag overrides whatever is in the config file. Messages that are already "Meow Meow Meow Meow" are skipped (no redundant edits on subsequent passes).
...
The `--skip-meowed` CLI flag overrides the config setting. Messages are matched by exact content (`"Meow Meow Meow Meow"`).
...
```
paracord/
├── paracord.py           # Main script (~830 lines)
